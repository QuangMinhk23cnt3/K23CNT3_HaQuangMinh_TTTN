...
export const getTasks = async (req: any, res: any, next: any) => {
  try {
    ...
    const tasks = await Task.find({ user: req.user.sub }).sort({ createdAt: -1 });
    
    // Transform (Biến đổi) _id của MongoDB thành id chuẩn cho Frontend dễ đọc
    const formattedTasks = tasks.map(task => {
      const obj = task.toObject() as any; // Chuyển Document Mongoose thành plain JavaScript Object
...
export const updateTask = async (req: any, res: any, next: any) => {
  try {
    ...
    const obj = (task as any).toObject() as any;
    obj.id = obj._id.toString();
    delete obj._id;
    delete obj.__v;
    if (obj.subtasks) {
